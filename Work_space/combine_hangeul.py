...
text = "안ㄴㅕㅇ하ㅏㅏ셍요 ㅅ111ㅂ 개ㅅㄲ야 '이런' 저런,, ~! ㅅ@ㅂ"
...
def get_prev(now_text, index, text_length):
    prev_form, prev_tag = None, None

    prev = (prev_form, prev_tag)

    # 문장의 첫 요소가 아닐 경우
    if index > 0:
        if text[index-1] != None:
            prev_form, prev_tag = split_text[index-1]
            prev = (prev_form, prev_tag)
...
def combine_word(now, prev, next, index):
    global right_point
                    ...
                    if n_next_form in MOUM:
                        ...
                        if not nn_next[1].startswith('S'):
                            ...
                            if nn_next_form in MOUM:
                                ...
                                print("nn_next is 자음")
                                print("now + next + n_next (자음 + 모음 + 자음)까지 저장")
                                combined_text.append(now_form + next_form + n_next)
                            else:
                                return
                    else:
                        return
...
for index, word in enumerate(split_text):
    if index < right_point:
        continue
    prev = get_prev(word, index, text_length)
    ...
    if not word[1].startswith('S'):
        ...
        continue
    combine_word(word, prev, next, index)
